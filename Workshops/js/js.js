//blok1
function mousein1() {
    document.getElementById('blok1').style.display = 'block';
    doxumwnr.getElementById('blok1').style
}
function mouseout1() {
    document.getElementById('blok1').style.display = 'none';
}
//blok2

function mousein2() {
    document.getElementById('blok2').style.display = 'block';
}
function mouseout2() {
    document.getElementById('blok2').style.display = 'none';
}


//blok3

function mousein3() {
    document.getElementById('blok3').style.display = 'block';
}
function mouseout3() {
    document.getElementById('blok3').style.display = 'none';
}


//blok4

function mousein4() {
    document.getElementById('blok4').style.display = 'block';
}
function mouseout4() {
    document.getElementById('blok4').style.display = 'none';
}

//blok5

function mousein5() {
    document.getElementById('blok5').style.display = 'block';
}
function mouseout5() {
    document.getElementById('blok5').style.display = 'none';
}

//blok6

function mousein6() {
    document.getElementById('blok6').style.display = 'block';
}
function mouseout6() {
    document.getElementById('blok6').style.display = 'none';
}

//blok7

function mousein7() {
    document.getElementById('blok7').style.display = 'block';
}
function mouseout7() {
    document.getElementById('blok7').style.display = 'none';
}

//blok8

function mousein8() {
    document.getElementById('blok8').style.display = 'block';
}
function mouseout8() {
    document.getElementById('blok8').style.display = 'none';
}

//blok9

function mousein9() {
    document.getElementById('blok9').style.display = 'block';
}
function mouseout9() {
    document.getElementById('blok9').style.display = 'none';
}